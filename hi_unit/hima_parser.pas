...
type                                            
  ...
  private
    ...
    function ReadToThen(var token: THimaToken): Boolean;
  ...
  try
    ...
    begin
      ...
      case token.TokenID of
        ...
        {IF} token_if:
          begin
            TokenNextToken(token); // SKIP "IF"
            TokenSkipComma(token);
            flagMosi := True;
            tmp := token;
            if _chk(ReadToThen(token),token_if) then Break else token := tmp;
          end;
        {代入} token_dainyu:
          begin
...
begin
  ...
  begin
    // 条件を１つ読む
    if not ReadOneItem(token) then raise HException.Create('IF condition could not be read.');
    if (token <> nil) and (token.TokenType = tokenOperator) then
    begin
      if not ReadOneItem(token) then raise HException.Create('IF condition could not be read.');
    end;
    ...
    // ...thenなら抜ける
    if (A.JosiId = josi_then)or(A.JosiId = josi_naraba)or(A.JosiId = josi_denakereba) then Break;
    ...
    // その他の条件
    if (token <> nil) and (token.TokenID = token_then) then Break;
  end;
